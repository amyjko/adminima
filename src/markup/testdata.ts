...
export function markup(next: () => number): Markup {
...
/** A fixed corpus of generated trees, so that every test sees the same cases. */
export function corpus(count: number): Markup[] {
	return Array.from({ length: count }, (_, seed) => markup(random(seed + 1)));
}
